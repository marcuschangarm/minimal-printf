...
{
    return mbed_minimal_formatted_string(buffer, LONG_MAX, format, arguments);
}
...
int SUB_FPRINTF(FILE* stream, const char* format, ...)
{
    va_list arguments;
    va_start(arguments, format);
    int result = mbed_minimal_formatted_file(stream, format, arguments);
    va_end(arguments);
...
{
    return mbed_minimal_formatted_file(stream, format, arguments);
}
#endif
